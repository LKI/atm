package com.liriansu.atm.util;

/**
 * {@link MSG} is used for internationalization.
 */
public class MSG {
    // TODO use ResourceBundle to read messages
    public static final String NO_SUCH_PROCESS    = "There is no such process [%s]";
    public static final String FAIL_TO_SETUP_CMDS = "Fail to setup commands.";
    public static final String FAIL_TO_FETCH_HTML = "Fail to query html!";
    public static final String FAIL_TO_START_PS   = "Fail to start process!";
    public static final String STH_WRONG          = "Something wrong, usually code issue"; // TODO report this to central
}
